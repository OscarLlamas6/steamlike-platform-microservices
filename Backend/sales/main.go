...
func Saludo(c *gin.Context) {
	c.JSON(http.StatusOK, gin.H{"data": "Microservicio Sales - Steamlike Platform | Grupo 4 :D"})
}
...
func main() {
	...
	sales := r.Group("/sales")
	{
		sales.POST("/create", authMiddleware.MiddlewareFunc(), controllers.CreateSale())
		sales.PUT("/update", authMiddleware.MiddlewareFunc(), controllers.UpdateSale())
		sales.GET("/single/:idSale", authMiddleware.MiddlewareFunc(), controllers.GetSale())
		sales.GET("/list/:username", authMiddleware.MiddlewareFunc(), controllers.GetSalesByUser())
		sales.GET("/list/all", authMiddleware.MiddlewareFunc(), controllers.GetAllSales())
		sales.DELETE("/delete/:idSale", authMiddleware.MiddlewareFunc(), controllers.DeleteSale())
	}
