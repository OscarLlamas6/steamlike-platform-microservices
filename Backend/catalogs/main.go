...
func Saludo(c *gin.Context) {
	c.JSON(http.StatusOK, gin.H{"data": "Microservicios de catalogos - Steamlike Platform | Grupo 4 :D"})
}
...
func main() {
	...
	{
		category.POST("/create", authMiddleware.MiddlewareFunc(), controllers.CreateCategory())
		category.GET("/:idCategory", authMiddleware.MiddlewareFunc(), controllers.GetCategory())
		category.GET("/list", authMiddleware.MiddlewareFunc(), controllers.GetCategories())
		category.GET("/list/noauth", controllers.GetCategories())
		category.PUT("/update", authMiddleware.MiddlewareFunc(), controllers.UpdateCategory())
		category.DELETE("/delete/:idCategory", authMiddleware.MiddlewareFunc(), controllers.DeleteCategory())
	}

	region := r.Group("/region")
	{
		region.POST("/create", authMiddleware.MiddlewareFunc(), controllers.CreateRegion())
		region.GET("/:idRegion", authMiddleware.MiddlewareFunc(), controllers.GetRegion())
		region.GET("/list", authMiddleware.MiddlewareFunc(), controllers.GetRegions())
		region.PUT("/update", authMiddleware.MiddlewareFunc(), controllers.UpdateRegion())
		region.DELETE("/delete/:idRegion", authMiddleware.MiddlewareFunc(), controllers.DeleteRegion())
	}
