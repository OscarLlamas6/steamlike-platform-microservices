...
func Saludo(c *gin.Context) {
	c.JSON(http.StatusOK, gin.H{"data": "Microservicios MyGames - Steamlike Platform | Grupo 4 :D"})
}
...
func main() {
	...
	{
		library.POST("/create", authMiddleware.MiddlewareFunc(), controllers.CreateLibraryGame())
		library.GET("/:username", authMiddleware.MiddlewareFunc(), controllers.GetMyGames())
		library.PUT("/update", authMiddleware.MiddlewareFunc(), controllers.UpdateMyGame())
		library.DELETE("/delete/:idMyGame", authMiddleware.MiddlewareFunc(), controllers.DeleteMyGame())
		library.DELETE("/remove/:idMyGame", authMiddleware.MiddlewareFunc(), controllers.RemoveMyGame())
	}
