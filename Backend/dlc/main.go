...
func Saludo(c *gin.Context) {
	c.JSON(http.StatusOK, gin.H{"data": "Microservicio DLC - Steamlike Platform | Grupo 4 :D"})
}
...
func main() {
	...
	dlc := r.Group("/dlc")
	{
		dlc.POST("/create", authMiddleware.MiddlewareFunc(), controllers.CreateDLC())
		dlc.PUT("/update", authMiddleware.MiddlewareFunc(), controllers.UpdateDLC())
		dlc.GET("/single/:idDLC", authMiddleware.MiddlewareFunc(), controllers.GetDLC())
		dlc.GET("/single/noauth/:idDLC", controllers.GetDLC())
		dlc.GET("/info/:idDLC", authMiddleware.MiddlewareFunc(), controllers.GetDLCAdmin())
		dlc.GET("/list", authMiddleware.MiddlewareFunc(), controllers.GetDLCs())
		dlc.GET("/game/list/:idGame", authMiddleware.MiddlewareFunc(), controllers.GetDLCsByGame())
		dlc.GET("/game/list/:idGame/noauth", controllers.GetDLCsByGameNoAuth())
		dlc.GET("/list/noauth", controllers.GetDLCs())
		dlc.GET("/list/all", authMiddleware.MiddlewareFunc(), controllers.GetAllDLCs())
		dlc.DELETE("/delete/:idDLC", authMiddleware.MiddlewareFunc(), controllers.DeleteDLC())
		dlc.GET("/restore/:idDLC", authMiddleware.MiddlewareFunc(), controllers.RestoreDLC())
	}
