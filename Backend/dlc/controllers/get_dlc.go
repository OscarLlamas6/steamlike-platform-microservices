...
func GetDLC() gin.HandlerFunc {
	...
	return func(c *gin.Context) {
		...
		DLCListQuery, err := db.Query("SELECT idDLC, name, idGame, isDeleted, imageURL, description, releaseDate, isGlobal, globalPrice, globalDiscount FROM DLC WHERE idDLC = ? AND isDeleted = 0;", idDLC)
		if err != nil {
			defer db.Close()
