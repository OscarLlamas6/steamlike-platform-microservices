...
func Saludo(c *gin.Context) {
	c.JSON(http.StatusOK, gin.H{"data": "Microservicios Usuarios - Steamlike Platform | Grupo 4 :D"})
}
...
func main() {
	...
	user := r.Group("/user")
	{
		...
		user.GET("/profile/:username", authMiddleware.MiddlewareFunc(), controllers.GetProfile())
		user.GET("/list/all", authMiddleware.MiddlewareFunc(), controllers.GetAllUsers())
		user.PUT("/update", authMiddleware.MiddlewareFunc(), controllers.UpdateUser())
		user.DELETE("/delete/:username", authMiddleware.MiddlewareFunc(), controllers.DeleteUser())
		user.GET("/restore/:username", authMiddleware.MiddlewareFunc(), controllers.RestoreUser())
		user.GET("/verify", controllers.VerifyUser())
		user.GET("/resend/verify", controllers.ResendVerify())
