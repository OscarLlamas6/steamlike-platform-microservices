...
func GetGamesByAgeRestriction() gin.HandlerFunc {

	return func(c *gin.Context) {

		claims := jwt.ExtractClaims(c)
		regionParam := claims["region"].(string)
		birthDate := c.DefaultQuery("birthDate", "1970-01-01")
		userAge := utils.CalculateAge(birthDate)
		...
		for gameListQuery.Next() {
			...
			if utils.GameAllowed(userAge, myGameAux.RestrictionAge) {
